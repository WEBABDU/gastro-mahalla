...
export const OrganizationCard = ({ card, match }) => {
  
  console.log("orgcard", match);
    ...
    <>
      <Col lg={5} md={12} className={style.cardBlock}>
        <Row className={style.cardDes}>
          <Col lg={6} sm={6}>
            <NavLink to={`${match.url}/${card.id}/${card.title}`} className={style.cardImg} onClick={()=>window.scroll(0,0)}>
              <img src={!card.image.thumbnails.medium ? noImage : card.image.thumbnails.medium} alt="img" />
            </NavLink>
          </Col>
          <Col lg={6} sm={6} className={style.cardBody}>
            <NavLink to={`${match.url}/${card.id}/${card.title}`} onClick={()=>window.scroll(0,0)}>
